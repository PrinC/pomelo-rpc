...
var Acceptor = function(opts, cb){
  ...
  // sequenceMap for clients, at most once rpc semantic
  this.sequenceMap = {};
};
  ...
  this.server.sockets.on('connection', function(socket) {
    ...
    socket.on('message', function(pkg) {
      try {
        ...
        var err = new RpcError(util.format('Server exception, msg: %s', e.msg), constants.RPC_ERROR_CODE.SERVER_EXCEPTION);
        var resp = [cloneError(err)];
        resp = {id: pkg.id, resp: resp};
...
pro.handleSequence = function(pkg, id, tracer) {
  if (this.sequenceMap[id] == undefined) {
    this.sequenceMap[id] = -1;
  }
  if (pkg.msg.seq == (this.sequenceMap[id] + 1)) {
    this.sequenceMap[id] ++;
    return true;
  }
  else {
    var err = new RpcError(util.format('Error sequence, current sequence number: %s, request\'s sequence number: %s', this.sequenceMap[id], pkg.msg.seq), constants.RPC_ERROR_CODE.ERROR_SEQUENCE);
    var resp = [cloneError(err)];
    if(tracer.isEnabled) {
      resp = {traceId: tracer.id, seqId: tracer.seq, source: tracer.source, id: pkg.id, resp: resp};
    }
    else {
      resp = {id: pkg.id, resp: resp};
    }
    this.sockets[id].emit('message', resp);
    return false;
  }
}
...
var cloneError = function(origin) {
  ...
  if (!(origin.code == undefined))
    res.code = origin.code;
  return res;
};
...
var processMsg = function(socket, acceptor, pkg) {
  ...
  if (!acceptor.handleSequence(pkg, socket.id, tracer)) {
    return;
  }
